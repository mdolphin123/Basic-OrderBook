...
struct InputHandler { //Converts from text inputs into C++ orderbook actions/objects
    private:
        ...
        bool TryParseResult(const std::string_view& str, Result& result) const { //take test from text file and turn it into real numeric values, 
            //pass in an empty result struct, then fill it!
            if(str.at(0) != 'R') {
                return false;
            }
        ...
        bool TryParseInformation (const std::string_view& str, Information& info) const { //Converts from text to information struct
            auto value = str.at(0); //take the first character of input line, command we are doing
            auto values = Split(str, ' '); //now split the line
            if(value == 'A') { //add order!
                info.type_ = ActionType::Add;
        ...
        std::vector<std::string_view> Split(const std::string_view& str, char delimeter) const { //C++ has no split function!
            std::vector<std::string_view> columns{}; //empty vector columns that stores the pieces
            std::size_t startIndex{}, endIndex{}; //creates variables start and end index that track string position, unsigned
            while((endIndex = str.find(delimeter, startIndex)) && endIndex != std::string::npos) { //keep looping while we still have delimeters
                auto distance = endIndex - startIndex;
        ...
        Side ParseSide(const std::string_view& str) const { //converts into side enum in orderbook class
            if(str == "B") {
                return Side::Buy;
            }
            else if(str == "S") {
        ...
        OrderType ParseOrderType(const std::string_view& str) const { //for the OrderType enum
            if(str == "FillandKill") {
                return OrderType::FillandKill;
            }
            else if(str == "GoodTillCancel") {
        ...
        Price ParsePrice(const std::string_view& str) const { //parses price field from the test file, converts it to number
            if(str.empty()) {
                throw std::logic_error("Unknown Price");
            }
            return ToNumber(str);
        ...
        Quantity ParseQuantity(const std::string_view& str) const { //parses quantity from test file, converts it to number
            if(str.empty()) {
                throw std::logic_error("Unknown Quantity");
            }
            return ToNumber(str);
        ...
        OrderId ParseOrderId(const std::string_view& str) const { //parses orderId
            if(str.empty()) {
                throw std::logic_error("Unknown Quantity");
            }
            return ToNumber(str);
